...
//To Calculate Limiting Distribution
void limiting_distribution(vector<pair<double,int>> &pi, int n, int st){
	...
	do
	{	flag = 0;
		binary_exponentiation(n, st, k);
		vector<vector<double>> temp(prob_k);
		binary_exponentiation(n, st, k+1);
		for(int i=0;i<size;++i){
			for(int j=0;j<size;++j){
				if(abs(temp[i][j]-prob_k[i][j])>1e-9){
					flag = 1;
					break;
				}
...
int main(){
	...
	int generation = 1;
	int start_st = 0;
	vector<int> seed_gen = {1,0,0,1,1,0,0,0};
	...
	cout<<"Start State= "<<start_st<<endl<<endl;
	

	vector<int> generations = {5,15};
	for(int itr = 0; itr < generations.size(); ++itr) {
		generation = generations[itr];
		...
		cout<<"Most likely state is:  "<<pi[0].second<<" with probability= "<<pi[0].first<<endl;
		cout<<"Thus, Most likely configuration is:\n";
		cout<<"|";
		for(auto el : state[pi[0].second]){
			cout<<"| "<<el<<" |"; 
		}	
		cout<<"|"<<endl<<endl;
	...
	//Limiting Distribution, As proved mathematically that limiting distribution exists
	//we calculate transition matrix for very high generation value, generation = 1000, generation = 1001
	cout<<"k= Infinity"<<endl;
	vector<pair<double,int>> pi(total_states);
	limiting_distribution(pi,grid_size, start_st);
	sort(pi.begin(), pi.end(), greater<pair<double,int>>());
	cout<<"Most likely state is:  "<<pi[0].second<<" with probability= "<<pi[0].first<<endl;
	cout<<"Thus, Most likely configuration is:\n";
	for(auto el : state[pi[0].second]){
	...
	double max_prob = pi[0].first;
	int t_flag = 0;
	for(int i=1; i<total_states;++i){
		if(pi[i].first == max_prob){
			cout<<"Equally likely state is:  "<<pi[0].second<<" with probability= "<<pi[0].first<<endl;
			cout<<"Thus, the configuration is:\n";
			cout<<"|";
			for(auto el : state[pi[0].second]){
				cout<<"| "<<el<<" |"; 
			}
